...
resource "aws_ssm_parameter" "edo_db_endpoint_param" {
  ...
  type  = "String"
  value = data.terraform_remote_state.edo.outputs.edo_db_instance_address
...
resource "aws_ssm_parameter" "edo_db_port_param" {
  name  = "/edo/db/port"
  type  = "String"
  value = tostring(data.terraform_remote_state.edo.outputs.edo_db_instance_port)
...
resource "aws_secretsmanager_secret_version" "edo_db_secret_version01" {
  ...
  secret_string = jsonencode({
    ...
    host = data.terraform_remote_state.edo.outputs.edo_db_instance_address
    port = data.terraform_remote_state.edo.outputs.edo_db_instance_port
